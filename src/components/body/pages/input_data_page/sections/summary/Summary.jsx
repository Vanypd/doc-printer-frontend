...
const Summary = ({ patientData, dataSetter }) => {

    const arrayDataChanger = (itemKey, type, fieldData, event) => {
        if (fieldData == 'selectorValue') {
            let array = [...patientData.summary.clinicalAnalyzes]
            ...
            dataSetter.setClinicalAnalyzes(array)
        }
        else {
            let array = [...patientData.summary.clinicalAnalyzes]
    ...
    return (
        <section className={cl.data}>
            <Block_division>
                ...
                <Data_field data={patientData.summary.medicalHistoryNumber} setData={e => dataSetter.setMedicalHistoryNumber(e)} style={{ width: '200px', textAlign: 'center' }} />
            </Block_division>

            <Block_division>
                <div className={[cl.title, 'title_text_style'].join(' ')} style={{ width: '175px' }}>Дата выписки:</div>
                <Data_field data={patientData.summary.dateOfDischarge} setData={e => dataSetter.setDateOfDischarge(e)} style={{ width: '200px', textAlign: 'center' }} />
            </Block_division>

            <Block_division>
                <div className={[cl.title, 'title_text_style'].join(' ')} style={{ width: '175px' }}>Трудоспособность:</div>
                <Data_field data={patientData.summary.workAbility} setData={e => dataSetter.setWorkAbility(e)} />
            </Block_division>
            ...
            <Block_division>
                <Data_textarea data={patientData.summary.instrumentalExaminationMethods} setData={e => dataSetter.setInstrumentalExaminationMethods(e)} style={{ minHeight: '300px' }} />
            </Block_division>
            ...
            {patientData.summary.clinicalAnalyzes.map(analyze =>
                <Analyzes key={analyze.itemKey} itemKey={analyze.itemKey} type={analyze.selectorValue} data={patientData.summary.clinicalAnalyzes} setData={arrayDataChanger} arraySetter={dataSetter.setClinicalAnalyzes} />
            )}

            <div className={cl.analyze_adder_block}>
                <button onClick={addAnalyze} className={cl.analyze_adder_button}>
                    <div>
            ...
            <Block_division>
                <Data_textarea data={patientData.summary.recomendations} setData={e => dataSetter.setRecomendations(e)} style={{ minHeight: '300px' }} />
            </Block_division>
        </section>
    )
}
